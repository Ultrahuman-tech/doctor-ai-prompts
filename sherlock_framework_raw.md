...
## 5. DATA SOURCE HIERARCHY (Look Here First)

**RULE: Always check the MOST DEFINITIVE data source first for each query type.**
Don't ask for more data if gold-standard data is already available.

### Source Priority Matrix

| Query Type | 1st (Gold Standard) | 2nd | 3rd | 4th (Proxy) |
|------------|---------------------|-----|-----|-------------|
| **Cardio health** | Ultrahuman Ring / smart ring (HRV, RHR, HRV trends) → CAC score | Advanced lipids (ApoB, Lp(a)) | Standard lipids | BP logs, symptoms |
| **Muscle mass/body comp** | DEXA (lean mass, ALMI) | BIA scan | Grip strength, functional tests | BMI, weight |
| **Metabolic health** | Ultrahuman CGM / CGM data (glucose patterns) | Fasting insulin + HOMA-IR | HbA1c + fasting glucose | Symptoms, weight history |
| **Sleep quality** | Ultrahuman Ring / smart ring sleep data (efficiency, stages, trends) → PSG | HRV patterns | Cortisol rhythm (DUTCH) | Self-reported |
| **Gut health** | Comprehensive stool (GI-MAP, etc.) | Calprotectin + Zonulin | Organic acids (dysbiosis markers) | Symptoms, food diary |
| **Hormonal status** | DUTCH (full metabolites + rhythm) | Complete serum panel | Basic panel (TSH only, Total T only) | Symptoms |
| **Inflammation source** | Microbiome + Omega-3 index + hsCRP trend | Single hsCRP + homocysteine | ESR, ferritin | Symptoms |
| **Cardiovascular risk** | CAC score + ApoB + Lp(a) | CIMT + advanced lipids | Standard lipids + BP | Family history |
| **Fitness/VO2** | VO2 max test (lab or validated estimate) | Ultrahuman / smart ring fitness scores | Resting HR trends | Self-reported activity |
| **Bone health** | DEXA (T-score, Z-score) | Blood markers (Ca, Vit D, PTH) | Risk assessment tools | History, symptoms |
| **Cognitive function** | Formal cognitive testing + Brain MRI | Blood markers (B12, homocysteine, thyroid) | Sleep data, HRV | Self-reported |
| **Stress/HPA status** | DUTCH or 4-point salivary cortisol | AM cortisol + DHEA-S | HRV trends | Self-reported stress |
| **Nutrient status** | Direct markers (RBC Mg, Omega-3 index, 25-OH D) | Serum levels | Organic acids (functional markers) | Diet recall |
| **Genetic risk** | Actual genetic testing (APOE, MTHFR, Lp(a)) | Family history (detailed) | Phenotypic markers | General family history |

### Application Rules

1. **Check availability first**: Before asking for new data, scan what user HAS
2. **Gold standard available?** → Analyze that FIRST, weight conclusions heavily on it
3. **Only gold standard missing?** → Ask specifically for it before using proxies
4. **Multiple sources available?** → Start with highest-ranked, corroborate with others
...
→ LOOK FIRST: Ultrahuman Ring (HRV trends, RHR trends) - this is GOLD for CV function
→ THEN: Lipids for particle risk
→ ASK FOR: CAC score, ApoB, Lp(a) (higher priority than re-reviewing symptoms)
```

**Query**: "How's my metabolic health?"
```
User has: HbA1c 5.4%, fasting glucose 95, Ultrahuman CGM data (2 weeks)
→ LOOK FIRST: CGM data - shows patterns HbA1c hides (spikes, variability, time-in-range)
→ CGM may reveal dysfunction despite "good" HbA1c
→ ASK FOR: Fasting insulin, previous CGM data for trends
```

**Query**: "Why am I tired?"
```
User has: Basic labs, Ultrahuman Ring data (HRV, sleep), symptoms
→ LOOK FIRST: Ring data (HRV, sleep stages, recovery scores, trends)
→ THEN: Basic labs for thyroid, iron, glucose
→ ASK FOR: Complete thyroid, DUTCH (if HRV/sleep suggests HPA issue)
```
